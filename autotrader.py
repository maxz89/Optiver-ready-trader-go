...
class AutoTrader(BaseAutoTrader):
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            if self.position < -10 and volume > -1 * self.position:
                self.time_of_last_hedged = self.event_loop.time()
            ...
            if self.position > 10 and volume > self.position:
                self.time_of_last_hedged = self.event_loop.time()
            self.position -= volume
        ...
        self.logger.info("new position is %d and new hedge position is %d", self.position, self.hedge_position)
